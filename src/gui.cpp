...
static void DrawMenuBar(void)
{
	if (ImGui::BeginMainMenuBar())
	{
		...
		if (gScene && ImGui::BeginMenu("Tools"))
		{
			...
			if (ImGui::MenuItem("Move Scene Textures to External Object File"))
			{
				const char *fn = noc_file_dialog_open(NOC_FILE_DIALOG_SAVE, "texture bank files (.zobj)\0*.zobj\0", NULL, NULL);
				
				if (fn)
				{
					const char *defaultName = "recipe";
					char *buf = StrdupPad(fn, 100);
					char *objName;
					FILE *obj = fopen(fn, "wb");
					FILE *recipe = stdout;
					
					// /path/to/object.zobj\0 -> /path/to\0object.zobj\0
					// object.zobj\0 -> \0bject.zobj\0
					*MAX3(strrchr(buf, '/'), strrchr(buf, '\\'), buf) = '\0';
					objName = Strdup(*buf ? buf + strlen(buf) + 1 : fn);
					
					// prepare path to recipe folder
					if (*buf) strcat(buf, "/"); // full path
					strcat(buf, defaultName);
					
					// open recipe file for writing
					char *tmp = buf + strlen(buf);
					strcat(tmp, ".txt");
					recipe = fopen(buf, "w");
					strcpy(tmp, "/"); // revert filename to dirname
					
					// write the header
					fprintf(recipe, "# generated by z64scene\n");
					fprintf(recipe, "*none # must start with * (other options: dither, random-dither)\n");
					fprintf(recipe, "%s\n", objName); // dest
					fprintf(recipe, "%s/\n", defaultName); // src
					
					// initialize writer
					DataBlobListMakeTextureBank(NULL, NULL, NULL, NULL);
					
					// write
					DataBlobListMakeTextureBank(gScene->blobs, obj, recipe, buf);
					sb_foreach(gScene->rooms, {
						//printf("room %p\n", each);
						DataBlobListMakeTextureBank(each->blobs, obj, recipe, buf);
					})
					...
					free(objName);
					fclose(obj);
					if (recipe != stdout) fclose(recipe);
				}
			}
			ImGui::EndMenu();
		}
